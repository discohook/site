export interface Emoji {
  names: string[]
  hasDiversity?: boolean
}

export const diversities = ["🏻", "🏼", "🏽", "🏾", "🏿"]

export const emojis: Record<string, Emoji> = {
  "😀": { names: ["grinning"] },
  "😬": { names: ["grimacing"] },
  "😁": { names: ["grin"] },
  "😂": { names: ["joy"] },
  "😃": { names: ["smiley"] },
  "😄": { names: ["smile"] },
  "😅": { names: ["sweat_smile"] },
  "😆": { names: ["laughing", "satisfied"] },
  "😇": { names: ["innocent"] },
  "😉": { names: ["wink"] },
  "😊": { names: ["blush"] },
  "🙂": { names: ["slight_smile", "slightly_smiling_face"] },
  "🙃": { names: ["upside_down", "upside_down_face"] },
  "☺": { names: ["relaxed"] },
  "😋": { names: ["yum"] },
  "😌": { names: ["relieved"] },
  "😍": { names: ["heart_eyes"] },
  "😘": { names: ["kissing_heart"] },
  "😗": { names: ["kissing"] },
  "😙": { names: ["kissing_smiling_eyes"] },
  "😚": { names: ["kissing_closed_eyes"] },
  "😜": { names: ["stuck_out_tongue_winking_eye"] },
  "😝": { names: ["stuck_out_tongue_closed_eyes"] },
  "😛": { names: ["stuck_out_tongue"] },
  "🤑": { names: ["money_mouth", "money_mouth_face"] },
  "🤓": { names: ["nerd", "nerd_face"] },
  "😎": { names: ["sunglasses"] },
  "🤗": { names: ["hugging", "hugging_face"] },
  "😏": { names: ["smirk"] },
  "😶": { names: ["no_mouth"] },
  "😐": { names: ["neutral_face"] },
  "😑": { names: ["expressionless"] },
  "😒": { names: ["unamused"] },
  "🙄": { names: ["rolling_eyes", "face_with_rolling_eyes"] },
  "🤔": { names: ["thinking", "thinking_face"] },
  "😳": { names: ["flushed"] },
  "😞": { names: ["disappointed"] },
  "😟": { names: ["worried"] },
  "😠": { names: ["angry"] },
  "😡": { names: ["rage"] },
  "😔": { names: ["pensive"] },
  "😕": { names: ["confused"] },
  "🙁": { names: ["slight_frown", "slightly_frowning_face"] },
  "☹": { names: ["frowning2", "white_frowning_face"] },
  "😣": { names: ["persevere"] },
  "😖": { names: ["confounded"] },
  "😫": { names: ["tired_face"] },
  "😩": { names: ["weary"] },
  "😤": { names: ["triumph"] },
  "😮": { names: ["open_mouth"] },
  "😱": { names: ["scream"] },
  "😨": { names: ["fearful"] },
  "😰": { names: ["cold_sweat"] },
  "😯": { names: ["hushed"] },
  "😦": { names: ["frowning"] },
  "😧": { names: ["anguished"] },
  "😢": { names: ["cry"] },
  "😥": { names: ["disappointed_relieved"] },
  "😪": { names: ["sleepy"] },
  "😓": { names: ["sweat"] },
  "😭": { names: ["sob"] },
  "😵": { names: ["dizzy_face"] },
  "😲": { names: ["astonished"] },
  "🤐": { names: ["zipper_mouth", "zipper_mouth_face"] },
  "😷": { names: ["mask"] },
  "🤒": { names: ["thermometer_face", "face_with_thermometer"] },
  "🤕": { names: ["head_bandage", "face_with_head_bandage"] },
  "😴": { names: ["sleeping"] },
  "💤": { names: ["zzz"] },
  "💩": { names: ["poop", "shit", "hankey", "poo"] },
  "😈": { names: ["smiling_imp"] },
  "👿": { names: ["imp"] },
  "👹": { names: ["japanese_ogre"] },
  "👺": { names: ["japanese_goblin"] },
  "💀": { names: ["skull", "skeleton"] },
  "👻": { names: ["ghost"] },
  "👽": { names: ["alien"] },
  "🤖": { names: ["robot", "robot_face"] },
  "😺": { names: ["smiley_cat"] },
  "😸": { names: ["smile_cat"] },
  "😹": { names: ["joy_cat"] },
  "😻": { names: ["heart_eyes_cat"] },
  "😼": { names: ["smirk_cat"] },
  "😽": { names: ["kissing_cat"] },
  "🙀": { names: ["scream_cat"] },
  "😿": { names: ["crying_cat_face"] },
  "😾": { names: ["pouting_cat"] },
  "🙌": { names: ["raised_hands"], hasDiversity: true },
  "👏": { names: ["clap"], hasDiversity: true },
  "👋": { names: ["wave"], hasDiversity: true },
  "👍": { names: ["thumbsup", "+1", "thumbup"], hasDiversity: true },
  "👎": { names: ["thumbsdown", "-1", "thumbdown"], hasDiversity: true },
  "👊": { names: ["punch"], hasDiversity: true },
  "✊": { names: ["fist"], hasDiversity: true },
  "✌": { names: ["v"], hasDiversity: true },
  "👌": { names: ["ok_hand"], hasDiversity: true },
  "✋": { names: ["raised_hand"], hasDiversity: true },
  "👐": { names: ["open_hands"], hasDiversity: true },
  "💪": { names: ["muscle"], hasDiversity: true },
  "🙏": { names: ["pray"], hasDiversity: true },
  "☝": { names: ["point_up"], hasDiversity: true },
  "👆": { names: ["point_up_2"], hasDiversity: true },
  "👇": { names: ["point_down"], hasDiversity: true },
  "👈": { names: ["point_left"], hasDiversity: true },
  "👉": { names: ["point_right"], hasDiversity: true },
  "🖕": {
    names: ["middle_finger", "reversed_hand_with_middle_finger_extended"],
    hasDiversity: true,
  },
  "🖐": {
    names: ["hand_splayed", "raised_hand_with_fingers_splayed"],
    hasDiversity: true,
  },
  "🤘": { names: ["metal", "sign_of_the_horns"], hasDiversity: true },
  "🖖": {
    names: ["vulcan", "raised_hand_with_part_between_middle_and_ring_fingers"],
    hasDiversity: true,
  },
  "✍": { names: ["writing_hand"], hasDiversity: true },
  "💅": { names: ["nail_care"], hasDiversity: true },
  "👄": { names: ["lips"] },
  "👅": { names: ["tongue"] },
  "👂": { names: ["ear"], hasDiversity: true },
  "👃": { names: ["nose"], hasDiversity: true },
  "👁": { names: ["eye"] },
  "👀": { names: ["eyes"] },
  "👤": { names: ["bust_in_silhouette"] },
  "👥": { names: ["busts_in_silhouette"] },
  "🗣": { names: ["speaking_head", "speaking_head_in_silhouette"] },
  "👶": { names: ["baby"], hasDiversity: true },
  "👦": { names: ["boy"], hasDiversity: true },
  "👧": { names: ["girl"], hasDiversity: true },
  "👨": { names: ["man"], hasDiversity: true },
  "👩": { names: ["woman"], hasDiversity: true },
  "👱": { names: ["person_with_blond_hair"], hasDiversity: true },
  "👴": { names: ["older_man"], hasDiversity: true },
  "👵": { names: ["older_woman", "grandma"], hasDiversity: true },
  "👲": { names: ["man_with_gua_pi_mao"], hasDiversity: true },
  "👳": { names: ["man_with_turban"], hasDiversity: true },
  "👮": { names: ["cop"], hasDiversity: true },
  "👷": { names: ["construction_worker"], hasDiversity: true },
  "💂": { names: ["guardsman"], hasDiversity: true },
  "🕵": { names: ["spy", "sleuth_or_spy"], hasDiversity: true },
  "🎅": { names: ["santa"], hasDiversity: true },
  "👼": { names: ["angel"], hasDiversity: true },
  "👸": { names: ["princess"], hasDiversity: true },
  "👰": { names: ["bride_with_veil"], hasDiversity: true },
  "🚶": { names: ["walking"], hasDiversity: true },
  "🏃": { names: ["runner"], hasDiversity: true },
  "💃": { names: ["dancer"], hasDiversity: true },
  "👯": { names: ["dancers"] },
  "👫": { names: ["couple"] },
  "👬": { names: ["two_men_holding_hands"] },
  "👭": { names: ["two_women_holding_hands"] },
  "🙇": { names: ["bow"], hasDiversity: true },
  "💁": { names: ["information_desk_person"], hasDiversity: true },
  "🙅": { names: ["no_good"], hasDiversity: true },
  "🙆": { names: ["ok_woman"], hasDiversity: true },
  "🙋": { names: ["raising_hand"], hasDiversity: true },
  "🙎": { names: ["person_with_pouting_face"], hasDiversity: true },
  "🙍": { names: ["person_frowning"], hasDiversity: true },
  "💇": { names: ["haircut"], hasDiversity: true },
  "💆": { names: ["massage"], hasDiversity: true },
  "💑": { names: ["couple_with_heart"] },
  "👩‍❤️‍👩": { names: ["couple_ww", "couple_with_heart_ww"] },
  "👨‍❤️‍👨": { names: ["couple_mm", "couple_with_heart_mm"] },
  "💏": { names: ["couplekiss"] },
  "👩‍❤️‍💋‍👩": { names: ["kiss_ww", "couplekiss_ww"] },
  "👨‍❤️‍💋‍👨": { names: ["kiss_mm", "couplekiss_mm"] },
  "👪": { names: ["family"] },
  "👨‍👩‍👧": { names: ["family_mwg"] },
  "👨‍👩‍👧‍👦": { names: ["family_mwgb"] },
  "👨‍👩‍👦‍👦": { names: ["family_mwbb"] },
  "👨‍👩‍👧‍👧": { names: ["family_mwgg"] },
  "👩‍👩‍👦": { names: ["family_wwb"] },
  "👩‍👩‍👧": { names: ["family_wwg"] },
  "👩‍👩‍👧‍👦": { names: ["family_wwgb"] },
  "👩‍👩‍👦‍👦": { names: ["family_wwbb"] },
  "👩‍👩‍👧‍👧": { names: ["family_wwgg"] },
  "👨‍👨‍👦": { names: ["family_mmb"] },
  "👨‍👨‍👧": { names: ["family_mmg"] },
  "👨‍👨‍👧‍👦": { names: ["family_mmgb"] },
  "👨‍👨‍👦‍👦": { names: ["family_mmbb"] },
  "👨‍👨‍👧‍👧": { names: ["family_mmgg"] },
  "👚": { names: ["womans_clothes"] },
  "👕": { names: ["shirt"] },
  "👖": { names: ["jeans"] },
  "👔": { names: ["necktie"] },
  "👗": { names: ["dress"] },
  "👙": { names: ["bikini"] },
  "👘": { names: ["kimono"] },
  "💄": { names: ["lipstick"] },
  "💋": { names: ["kiss"] },
  "👣": { names: ["footprints"] },
  "👠": { names: ["high_heel"] },
  "👡": { names: ["sandal"] },
  "👢": { names: ["boot"] },
  "👞": { names: ["mans_shoe"] },
  "👟": { names: ["athletic_shoe"] },
  "👒": { names: ["womans_hat"] },
  "🎩": { names: ["tophat"] },
  "⛑": { names: ["helmet_with_cross", "helmet_with_white_cross"] },
  "🎓": { names: ["mortar_board"] },
  "👑": { names: ["crown"] },
  "🎒": { names: ["school_satchel"] },
  "👝": { names: ["pouch"] },
  "👛": { names: ["purse"] },
  "👜": { names: ["handbag"] },
  "💼": { names: ["briefcase"] },
  "👓": { names: ["eyeglasses"] },
  "🕶": { names: ["dark_sunglasses"] },
  "💍": { names: ["ring"] },
  "🌂": { names: ["closed_umbrella"] },
  "🤠": { names: ["cowboy", "face_with_cowboy_hat"] },
  "🤡": { names: ["clown", "clown_face"] },
  "🤢": { names: ["nauseated_face", "sick"] },
  "🤣": { names: ["rofl", "rolling_on_the_floor_laughing"] },
  "🤤": { names: ["drooling_face", "drool"] },
  "🤥": { names: ["lying_face", "liar"] },
  "🤧": { names: ["sneezing_face", "sneeze"] },
  "🤴": { names: ["prince"], hasDiversity: true },
  "🤵": { names: ["man_in_tuxedo"], hasDiversity: true },
  "🤶": { names: ["mrs_claus", "mother_christmas"], hasDiversity: true },
  "🤦": { names: ["face_palm", "facepalm"], hasDiversity: true },
  "🤷": { names: ["shrug"], hasDiversity: true },
  "🤰": {
    names: ["pregnant_woman", "expecting_woman"],
    hasDiversity: true,
  },
  "🤳": { names: ["selfie"], hasDiversity: true },
  "🕺": { names: ["man_dancing", "male_dancer"], hasDiversity: true },
  "🤙": { names: ["call_me", "call_me_hand"], hasDiversity: true },
  "🤚": {
    names: ["raised_back_of_hand", "back_of_hand"],
    hasDiversity: true,
  },
  "🤛": { names: ["left_facing_fist", "left_fist"], hasDiversity: true },
  "🤜": {
    names: ["right_facing_fist", "right_fist"],
    hasDiversity: true,
  },
  "🤝": { names: ["handshake", "shaking_hands"] },
  "🤞": {
    names: ["fingers_crossed", "hand_with_index_and_middle_finger_crossed"],
    hasDiversity: true,
  },
  "🐶": { names: ["dog"] },
  "🐱": { names: ["cat"] },
  "🐭": { names: ["mouse"] },
  "🐹": { names: ["hamster"] },
  "🐰": { names: ["rabbit"] },
  "🐻": { names: ["bear"] },
  "🐼": { names: ["panda_face"] },
  "🐨": { names: ["koala"] },
  "🐯": { names: ["tiger"] },
  "🦁": { names: ["lion_face", "lion"] },
  "🐮": { names: ["cow"] },
  "🐷": { names: ["pig"] },
  "🐽": { names: ["pig_nose"] },
  "🐸": { names: ["frog"] },
  "🐙": { names: ["octopus"] },
  "🐵": { names: ["monkey_face"] },
  "🙈": { names: ["see_no_evil"] },
  "🙉": { names: ["hear_no_evil"] },
  "🙊": { names: ["speak_no_evil"] },
  "🐒": { names: ["monkey"] },
  "🐔": { names: ["chicken"] },
  "🐧": { names: ["penguin"] },
  "🐦": { names: ["bird"] },
  "🐤": { names: ["baby_chick"] },
  "🐣": { names: ["hatching_chick"] },
  "🐥": { names: ["hatched_chick"] },
  "🐺": { names: ["wolf"] },
  "🐗": { names: ["boar"] },
  "🐴": { names: ["horse"] },
  "🦄": { names: ["unicorn", "unicorn_face"] },
  "🐝": { names: ["bee"] },
  "🐛": { names: ["bug"] },
  "🐌": { names: ["snail"] },
  "🐞": { names: ["beetle"] },
  "🐜": { names: ["ant"] },
  "🕷": { names: ["spider"] },
  "🦂": { names: ["scorpion"] },
  "🦀": { names: ["crab"] },
  "🐍": { names: ["snake"] },
  "🐢": { names: ["turtle"] },
  "🐠": { names: ["tropical_fish"] },
  "🐟": { names: ["fish"] },
  "🐡": { names: ["blowfish"] },
  "🐬": { names: ["dolphin"] },
  "🐳": { names: ["whale"] },
  "🐋": { names: ["whale2"] },
  "🐊": { names: ["crocodile"] },
  "🐆": { names: ["leopard"] },
  "🐅": { names: ["tiger2"] },
  "🐃": { names: ["water_buffalo"] },
  "🐂": { names: ["ox"] },
  "🐄": { names: ["cow2"] },
  "🐪": { names: ["dromedary_camel"] },
  "🐫": { names: ["camel"] },
  "🐘": { names: ["elephant"] },
  "🐐": { names: ["goat"] },
  "🐏": { names: ["ram"] },
  "🐑": { names: ["sheep"] },
  "🐎": { names: ["racehorse"] },
  "🐖": { names: ["pig2"] },
  "🐀": { names: ["rat"] },
  "🐁": { names: ["mouse2"] },
  "🐓": { names: ["rooster"] },
  "🦃": { names: ["turkey"] },
  "🕊": { names: ["dove", "dove_of_peace"] },
  "🐕": { names: ["dog2"] },
  "🐩": { names: ["poodle"] },
  "🐈": { names: ["cat2"] },
  "🐇": { names: ["rabbit2"] },
  "🐿": { names: ["chipmunk"] },
  "🐾": { names: ["feet", "paw_prints"] },
  "🐉": { names: ["dragon"] },
  "🐲": { names: ["dragon_face"] },
  "🌵": { names: ["cactus"] },
  "🎄": { names: ["christmas_tree"] },
  "🌲": { names: ["evergreen_tree"] },
  "🌳": { names: ["deciduous_tree"] },
  "🌴": { names: ["palm_tree"] },
  "🌱": { names: ["seedling"] },
  "🌿": { names: ["herb"] },
  "☘": { names: ["shamrock"] },
  "🍀": { names: ["four_leaf_clover"] },
  "🎍": { names: ["bamboo"] },
  "🎋": { names: ["tanabata_tree"] },
  "🍃": { names: ["leaves"] },
  "🍂": { names: ["fallen_leaf"] },
  "🍁": { names: ["maple_leaf"] },
  "🌾": { names: ["ear_of_rice"] },
  "🌺": { names: ["hibiscus"] },
  "🌻": { names: ["sunflower"] },
  "🌹": { names: ["rose"] },
  "🌷": { names: ["tulip"] },
  "🌼": { names: ["blossom"] },
  "🌸": { names: ["cherry_blossom"] },
  "💐": { names: ["bouquet"] },
  "🍄": { names: ["mushroom"] },
  "🌰": { names: ["chestnut"] },
  "🎃": { names: ["jack_o_lantern"] },
  "🐚": { names: ["shell"] },
  "🕸": { names: ["spider_web"] },
  "🌎": { names: ["earth_americas"] },
  "🌍": { names: ["earth_africa"] },
  "🌏": { names: ["earth_asia"] },
  "🌕": { names: ["full_moon"] },
  "🌖": { names: ["waning_gibbous_moon"] },
  "🌗": { names: ["last_quarter_moon"] },
  "🌘": { names: ["waning_crescent_moon"] },
  "🌑": { names: ["new_moon"] },
  "🌒": { names: ["waxing_crescent_moon"] },
  "🌓": { names: ["first_quarter_moon"] },
  "🌔": { names: ["waxing_gibbous_moon"] },
  "🌚": { names: ["new_moon_with_face"] },
  "🌝": { names: ["full_moon_with_face"] },
  "🌛": { names: ["first_quarter_moon_with_face"] },
  "🌜": { names: ["last_quarter_moon_with_face"] },
  "🌞": { names: ["sun_with_face"] },
  "🌙": { names: ["crescent_moon"] },
  "⭐": { names: ["star"] },
  "🌟": { names: ["star2"] },
  "💫": { names: ["dizzy"] },
  "✨": { names: ["sparkles"] },
  "☄": { names: ["comet"] },
  "☀": { names: ["sunny"] },
  "🌤": { names: ["white_sun_small_cloud", "white_sun_with_small_cloud"] },
  "⛅": { names: ["partly_sunny"] },
  "🌥": { names: ["white_sun_cloud", "white_sun_behind_cloud"] },
  "🌦": {
    names: ["white_sun_rain_cloud", "white_sun_behind_cloud_with_rain"],
  },
  "☁": { names: ["cloud"] },
  "🌧": { names: ["cloud_rain", "cloud_with_rain"] },
  "⛈": { names: ["thunder_cloud_rain", "thunder_cloud_and_rain"] },
  "🌩": { names: ["cloud_lightning", "cloud_with_lightning"] },
  "⚡": { names: ["zap"] },
  "🔥": { names: ["fire", "flame"] },
  "💥": { names: ["boom"] },
  "❄": { names: ["snowflake"] },
  "🌨": { names: ["cloud_snow", "cloud_with_snow"] },
  "☃": { names: ["snowman2"] },
  "⛄": { names: ["snowman"] },
  "🌬": { names: ["wind_blowing_face"] },
  "💨": { names: ["dash"] },
  "🌪": { names: ["cloud_tornado", "cloud_with_tornado"] },
  "🌫": { names: ["fog"] },
  "☂": { names: ["umbrella2"] },
  "☔": { names: ["umbrella"] },
  "💧": { names: ["droplet"] },
  "💦": { names: ["sweat_drops"] },
  "🌊": { names: ["ocean"] },
  "🦅": { names: ["eagle"] },
  "🦆": { names: ["duck"] },
  "🦇": { names: ["bat"] },
  "🦈": { names: ["shark"] },
  "🦉": { names: ["owl"] },
  "🦊": { names: ["fox", "fox_face"] },
  "🦋": { names: ["butterfly"] },
  "🦌": { names: ["deer"] },
  "🦍": { names: ["gorilla"] },
  "🦎": { names: ["lizard"] },
  "🦏": { names: ["rhino", "rhinoceros"] },
  "🥀": { names: ["wilted_rose", "wilted_flower"] },
  "🦐": { names: ["shrimp"] },
  "🦑": { names: ["squid"] },
  "🍏": { names: ["green_apple"] },
  "🍎": { names: ["apple"] },
  "🍐": { names: ["pear"] },
  "🍊": { names: ["tangerine"] },
  "🍋": { names: ["lemon"] },
  "🍌": { names: ["banana"] },
  "🍉": { names: ["watermelon"] },
  "🍇": { names: ["grapes"] },
  "🍓": { names: ["strawberry"] },
  "🍈": { names: ["melon"] },
  "🍒": { names: ["cherries"] },
  "🍑": { names: ["peach"] },
  "🍍": { names: ["pineapple"] },
  "🍅": { names: ["tomato"] },
  "🍆": { names: ["eggplant"] },
  "🌶": { names: ["hot_pepper"] },
  "🌽": { names: ["corn"] },
  "🍠": { names: ["sweet_potato"] },
  "🍯": { names: ["honey_pot"] },
  "🍞": { names: ["bread"] },
  "🧀": { names: ["cheese", "cheese_wedge"] },
  "🍗": { names: ["poultry_leg"] },
  "🍖": { names: ["meat_on_bone"] },
  "🍤": { names: ["fried_shrimp"] },
  "🍳": { names: ["cooking"] },
  "🍔": { names: ["hamburger"] },
  "🍟": { names: ["fries"] },
  "🌭": { names: ["hotdog", "hot_dog"] },
  "🍕": { names: ["pizza"] },
  "🍝": { names: ["spaghetti"] },
  "🌮": { names: ["taco"] },
  "🌯": { names: ["burrito"] },
  "🍜": { names: ["ramen"] },
  "🍲": { names: ["stew"] },
  "🍥": { names: ["fish_cake"] },
  "🍣": { names: ["sushi"] },
  "🍱": { names: ["bento"] },
  "🍛": { names: ["curry"] },
  "🍙": { names: ["rice_ball"] },
  "🍚": { names: ["rice"] },
  "🍘": { names: ["rice_cracker"] },
  "🍢": { names: ["oden"] },
  "🍡": { names: ["dango"] },
  "🍧": { names: ["shaved_ice"] },
  "🍨": { names: ["ice_cream"] },
  "🍦": { names: ["icecream"] },
  "🍰": { names: ["cake"] },
  "🎂": { names: ["birthday"] },
  "🍮": { names: ["custard", "pudding", "flan"] },
  "🍬": { names: ["candy"] },
  "🍭": { names: ["lollipop"] },
  "🍫": { names: ["chocolate_bar"] },
  "🍿": { names: ["popcorn"] },
  "🍩": { names: ["doughnut"] },
  "🍪": { names: ["cookie"] },
  "🍺": { names: ["beer"] },
  "🍻": { names: ["beers"] },
  "🍷": { names: ["wine_glass"] },
  "🍸": { names: ["cocktail"] },
  "🍹": { names: ["tropical_drink"] },
  "🍾": { names: ["champagne", "bottle_with_popping_cork"] },
  "🍶": { names: ["sake"] },
  "🍵": { names: ["tea"] },
  "☕": { names: ["coffee"] },
  "🍼": { names: ["baby_bottle"] },
  "🍴": { names: ["fork_and_knife"] },
  "🍽": { names: ["fork_knife_plate", "fork_and_knife_with_plate"] },
  "🥐": { names: ["croissant"] },
  "🥑": { names: ["avocado"] },
  "🥒": { names: ["cucumber"] },
  "🥓": { names: ["bacon"] },
  "🥔": { names: ["potato"] },
  "🥕": { names: ["carrot"] },
  "🥖": { names: ["french_bread", "baguette_bread"] },
  "🥗": { names: ["salad", "green_salad"] },
  "🥘": { names: ["shallow_pan_of_food", "paella"] },
  "🥙": { names: ["stuffed_flatbread", "stuffed_pita"] },
  "🥂": { names: ["champagne_glass", "clinking_glass"] },
  "🥃": { names: ["tumbler_glass", "whisky"] },
  "🥄": { names: ["spoon"] },
  "🥚": { names: ["egg"] },
  "🥛": { names: ["milk", "glass_of_milk"] },
  "🥜": { names: ["peanuts", "shelled_peanut"] },
  "🥝": { names: ["kiwi", "kiwifruit"] },
  "🥞": { names: ["pancakes"] },
  "⚽": { names: ["soccer"] },
  "🏀": { names: ["basketball"] },
  "🏈": { names: ["football"] },
  "⚾": { names: ["baseball"] },
  "🎾": { names: ["tennis"] },
  "🏐": { names: ["volleyball"] },
  "🏉": { names: ["rugby_football"] },
  "🎱": { names: ["8ball"] },
  "⛳": { names: ["golf"] },
  "🏌": { names: ["golfer"], hasDiversity: true },
  "🏓": { names: ["ping_pong", "table_tennis"] },
  "🏸": { names: ["badminton"] },
  "🏒": { names: ["hockey"] },
  "🏑": { names: ["field_hockey"] },
  "🏏": { names: ["cricket", "cricket_bat_ball"] },
  "🎿": { names: ["ski"] },
  "⛷": { names: ["skier"], hasDiversity: true },
  "🏂": { names: ["snowboarder"], hasDiversity: true },
  "⛸": { names: ["ice_skate"] },
  "🏹": { names: ["bow_and_arrow", "archery"] },
  "🎣": { names: ["fishing_pole_and_fish"] },
  "🚣": { names: ["rowboat"], hasDiversity: true },
  "🏊": { names: ["swimmer"], hasDiversity: true },
  "🏄": { names: ["surfer"], hasDiversity: true },
  "🛀": { names: ["bath"], hasDiversity: true },
  "⛹": {
    names: ["basketball_player", "person_with_ball"],
    hasDiversity: true,
  },
  "🏋": { names: ["lifter", "weight_lifter"], hasDiversity: true },
  "🚴": { names: ["bicyclist"], hasDiversity: true },
  "🚵": { names: ["mountain_bicyclist"], hasDiversity: true },
  "🏇": { names: ["horse_racing"], hasDiversity: true },
  "🕴": {
    names: ["levitate", "man_in_business_suit_levitating"],
    hasDiversity: true,
  },
  "🏆": { names: ["trophy"] },
  "🎽": { names: ["running_shirt_with_sash"] },
  "🏅": { names: ["medal", "sports_medal"] },
  "🎖": { names: ["military_medal"] },
  "🎗": { names: ["reminder_ribbon"] },
  "🏵": { names: ["rosette"] },
  "🎫": { names: ["ticket"] },
  "🎟": { names: ["tickets", "admission_tickets"] },
  "🎭": { names: ["performing_arts"] },
  "🎨": { names: ["art"] },
  "🎪": { names: ["circus_tent"] },
  "🎤": { names: ["microphone"] },
  "🎧": { names: ["headphones"] },
  "🎼": { names: ["musical_score"] },
  "🎹": { names: ["musical_keyboard"] },
  "🎷": { names: ["saxophone"] },
  "🎺": { names: ["trumpet"] },
  "🎸": { names: ["guitar"] },
  "🎻": { names: ["violin"] },
  "🎬": { names: ["clapper"] },
  "🎮": { names: ["video_game"] },
  "👾": { names: ["space_invader"] },
  "🎯": { names: ["dart"] },
  "🎲": { names: ["game_die"] },
  "🎰": { names: ["slot_machine"] },
  "🎳": { names: ["bowling"] },
  "🤸": {
    names: ["cartwheel", "person_doing_cartwheel"],
    hasDiversity: true,
  },
  "🤹": { names: ["juggling", "juggler"], hasDiversity: true },
  "🤼": { names: ["wrestlers", "wrestling"] },
  "🥊": { names: ["boxing_glove", "boxing_gloves"] },
  "🥋": { names: ["martial_arts_uniform", "karate_uniform"] },
  "🤽": { names: ["water_polo"], hasDiversity: true },
  "🤾": { names: ["handball"], hasDiversity: true },
  "🥅": { names: ["goal", "goal_net"] },
  "🤺": { names: ["fencer", "fencing"] },
  "🥇": { names: ["first_place", "first_place_medal"] },
  "🥈": { names: ["second_place", "second_place_medal"] },
  "🥉": { names: ["third_place", "third_place_medal"] },
  "🥁": { names: ["drum", "drum_with_drumsticks"] },
  "🚗": { names: ["red_car"] },
  "🚕": { names: ["taxi"] },
  "🚙": { names: ["blue_car"] },
  "🚌": { names: ["bus"] },
  "🚎": { names: ["trolleybus"] },
  "🏎": { names: ["race_car", "racing_car"] },
  "🚓": { names: ["police_car"] },
  "🚑": { names: ["ambulance"] },
  "🚒": { names: ["fire_engine"] },
  "🚐": { names: ["minibus"] },
  "🚚": { names: ["truck"] },
  "🚛": { names: ["articulated_lorry"] },
  "🚜": { names: ["tractor"] },
  "🏍": { names: ["motorcycle", "racing_motorcycle"] },
  "🚲": { names: ["bike"] },
  "🚨": { names: ["rotating_light"] },
  "🚔": { names: ["oncoming_police_car"] },
  "🚍": { names: ["oncoming_bus"] },
  "🚘": { names: ["oncoming_automobile"] },
  "🚖": { names: ["oncoming_taxi"] },
  "🚡": { names: ["aerial_tramway"] },
  "🚠": { names: ["mountain_cableway"] },
  "🚟": { names: ["suspension_railway"] },
  "🚃": { names: ["railway_car"] },
  "🚋": { names: ["train"] },
  "🚝": { names: ["monorail"] },
  "🚄": { names: ["bullettrain_side"] },
  "🚅": { names: ["bullettrain_front"] },
  "🚈": { names: ["light_rail"] },
  "🚞": { names: ["mountain_railway"] },
  "🚂": { names: ["steam_locomotive"] },
  "🚆": { names: ["train2"] },
  "🚇": { names: ["metro"] },
  "🚊": { names: ["tram"] },
  "🚉": { names: ["station"] },
  "🚁": { names: ["helicopter"] },
  "🛩": { names: ["airplane_small", "small_airplane"] },
  "✈": { names: ["airplane"] },
  "🛫": { names: ["airplane_departure"] },
  "🛬": { names: ["airplane_arriving"] },
  "⛵": { names: ["sailboat"] },
  "🛥": { names: ["motorboat"] },
  "🚤": { names: ["speedboat"] },
  "⛴": { names: ["ferry"] },
  "🛳": { names: ["cruise_ship", "passenger_ship"] },
  "🚀": { names: ["rocket"] },
  "🛰": { names: ["satellite_orbital"] },
  "💺": { names: ["seat"] },
  "⚓": { names: ["anchor"] },
  "🚧": { names: ["construction"] },
  "⛽": { names: ["fuelpump"] },
  "🚏": { names: ["busstop"] },
  "🚦": { names: ["vertical_traffic_light"] },
  "🚥": { names: ["traffic_light"] },
  "🏁": { names: ["checkered_flag"] },
  "🚢": { names: ["ship"] },
  "🎡": { names: ["ferris_wheel"] },
  "🎢": { names: ["roller_coaster"] },
  "🎠": { names: ["carousel_horse"] },
  "🏗": { names: ["construction_site", "building_construction"] },
  "🌁": { names: ["foggy"] },
  "🗼": { names: ["tokyo_tower"] },
  "🏭": { names: ["factory"] },
  "⛲": { names: ["fountain"] },
  "🎑": { names: ["rice_scene"] },
  "⛰": { names: ["mountain"] },
  "🏔": { names: ["mountain_snow", "snow_capped_mountain"] },
  "🗻": { names: ["mount_fuji"] },
  "🌋": { names: ["volcano"] },
  "🗾": { names: ["japan"] },
  "🏕": { names: ["camping"] },
  "⛺": { names: ["tent"] },
  "🏞": { names: ["park", "national_park"] },
  "🛣": { names: ["motorway"] },
  "🛤": { names: ["railway_track", "railroad_track"] },
  "🌅": { names: ["sunrise"] },
  "🌄": { names: ["sunrise_over_mountains"] },
  "🏜": { names: ["desert"] },
  "🏖": { names: ["beach", "beach_with_umbrella"] },
  "🏝": { names: ["island", "desert_island"] },
  "🌇": { names: ["city_sunset", "city_sunrise"] },
  "🌆": { names: ["city_dusk"] },
  "🏙": { names: ["cityscape"] },
  "🌃": { names: ["night_with_stars"] },
  "🌉": { names: ["bridge_at_night"] },
  "🌌": { names: ["milky_way"] },
  "🌠": { names: ["stars"] },
  "🎇": { names: ["sparkler"] },
  "🎆": { names: ["fireworks"] },
  "🌈": { names: ["rainbow"] },
  "🏘": { names: ["homes", "house_buildings"] },
  "🏰": { names: ["european_castle"] },
  "🏯": { names: ["japanese_castle"] },
  "🏟": { names: ["stadium"] },
  "🗽": { names: ["statue_of_liberty"] },
  "🏠": { names: ["house"] },
  "🏡": { names: ["house_with_garden"] },
  "🏚": { names: ["house_abandoned", "derelict_house_building"] },
  "🏢": { names: ["office"] },
  "🏬": { names: ["department_store"] },
  "🏣": { names: ["post_office"] },
  "🏤": { names: ["european_post_office"] },
  "🏥": { names: ["hospital"] },
  "🏦": { names: ["bank"] },
  "🏨": { names: ["hotel"] },
  "🏪": { names: ["convenience_store"] },
  "🏫": { names: ["school"] },
  "🏩": { names: ["love_hotel"] },
  "💒": { names: ["wedding"] },
  "🏛": { names: ["classical_building"] },
  "⛪": { names: ["church"] },
  "🕌": { names: ["mosque"] },
  "🕍": { names: ["synagogue"] },
  "🕋": { names: ["kaaba"] },
  "⛩": { names: ["shinto_shrine"] },
  "🛴": { names: ["scooter"] },
  "🛵": { names: ["motor_scooter", "motorbike"] },
  "🛶": { names: ["canoe", "kayak"] },
  "⌚": { names: ["watch"] },
  "📱": { names: ["iphone"] },
  "📲": { names: ["calling"] },
  "💻": { names: ["computer"] },
  "⌨": { names: ["keyboard"] },
  "🖥": { names: ["desktop", "desktop_computer"] },
  "🖨": { names: ["printer"] },
  "🖱": { names: ["mouse_three_button", "three_button_mouse"] },
  "🖲": { names: ["trackball"] },
  "🕹": { names: ["joystick"] },
  "🗜": { names: ["compression"] },
  "💽": { names: ["minidisc"] },
  "💾": { names: ["floppy_disk"] },
  "💿": { names: ["cd"] },
  "📀": { names: ["dvd"] },
  "📼": { names: ["vhs"] },
  "📷": { names: ["camera"] },
  "📸": { names: ["camera_with_flash"] },
  "📹": { names: ["video_camera"] },
  "🎥": { names: ["movie_camera"] },
  "📽": { names: ["projector", "film_projector"] },
  "🎞": { names: ["film_frames"] },
  "📞": { names: ["telephone_receiver"] },
  "☎": { names: ["telephone"] },
  "📟": { names: ["pager"] },
  "📠": { names: ["fax"] },
  "📺": { names: ["tv"] },
  "📻": { names: ["radio"] },
  "🎙": { names: ["microphone2", "studio_microphone"] },
  "🎚": { names: ["level_slider"] },
  "🎛": { names: ["control_knobs"] },
  "⏱": { names: ["stopwatch"] },
  "⏲": { names: ["timer", "timer_clock"] },
  "⏰": { names: ["alarm_clock"] },
  "🕰": { names: ["clock", "mantlepiece_clock"] },
  "⏳": { names: ["hourglass_flowing_sand"] },
  "⌛": { names: ["hourglass"] },
  "📡": { names: ["satellite"] },
  "🔋": { names: ["battery"] },
  "🔌": { names: ["electric_plug"] },
  "💡": { names: ["bulb"] },
  "🔦": { names: ["flashlight"] },
  "🕯": { names: ["candle"] },
  "🗑": { names: ["wastebasket"] },
  "🛢": { names: ["oil", "oil_drum"] },
  "💸": { names: ["money_with_wings"] },
  "💵": { names: ["dollar"] },
  "💴": { names: ["yen"] },
  "💶": { names: ["euro"] },
  "💷": { names: ["pound"] },
  "💰": { names: ["moneybag"] },
  "💳": { names: ["credit_card"] },
  "💎": { names: ["gem"] },
  "⚖": { names: ["scales"] },
  "🔧": { names: ["wrench"] },
  "🔨": { names: ["hammer"] },
  "⚒": { names: ["hammer_pick", "hammer_and_pick"] },
  "🛠": { names: ["tools", "hammer_and_wrench"] },
  "⛏": { names: ["pick"] },
  "🔩": { names: ["nut_and_bolt"] },
  "⚙": { names: ["gear"] },
  "⛓": { names: ["chains"] },
  "🔫": { names: ["gun"] },
  "💣": { names: ["bomb"] },
  "🔪": { names: ["knife"] },
  "🗡": { names: ["dagger", "dagger_knife"] },
  "⚔": { names: ["crossed_swords"] },
  "🛡": { names: ["shield"] },
  "🚬": { names: ["smoking"] },
  "☠": { names: ["skull_crossbones", "skull_and_crossbones"] },
  "⚰": { names: ["coffin"] },
  "⚱": { names: ["urn", "funeral_urn"] },
  "🏺": { names: ["amphora"] },
  "🔮": { names: ["crystal_ball"] },
  "📿": { names: ["prayer_beads"] },
  "💈": { names: ["barber"] },
  "⚗": { names: ["alembic"] },
  "🔭": { names: ["telescope"] },
  "🔬": { names: ["microscope"] },
  "🕳": { names: ["hole"] },
  "💊": { names: ["pill"] },
  "💉": { names: ["syringe"] },
  "🌡": { names: ["thermometer"] },
  "🏷": { names: ["label"] },
  "🔖": { names: ["bookmark"] },
  "🚽": { names: ["toilet"] },
  "🚿": { names: ["shower"] },
  "🛁": { names: ["bathtub"] },
  "🔑": { names: ["key"] },
  "🗝": { names: ["key2", "old_key"] },
  "🛋": { names: ["couch", "couch_and_lamp"] },
  "🛌": { names: ["sleeping_accommodation"], hasDiversity: true },
  "🛏": { names: ["bed"] },
  "🚪": { names: ["door"] },
  "🛎": { names: ["bellhop", "bellhop_bell"] },
  "🖼": { names: ["frame_photo", "frame_with_picture"] },
  "🗺": { names: ["map", "world_map"] },
  "⛱": { names: ["beach_umbrella", "umbrella_on_ground"] },
  "🗿": { names: ["moyai"] },
  "🛍": { names: ["shopping_bags"] },
  "🎈": { names: ["balloon"] },
  "🎏": { names: ["flags"] },
  "🎀": { names: ["ribbon"] },
  "🎁": { names: ["gift"] },
  "🎊": { names: ["confetti_ball"] },
  "🎉": { names: ["tada"] },
  "🎎": { names: ["dolls"] },
  "🎐": { names: ["wind_chime"] },
  "🎌": { names: ["crossed_flags"] },
  "🏮": { names: ["izakaya_lantern"] },
  "✉": { names: ["envelope"] },
  "📩": { names: ["envelope_with_arrow"] },
  "📨": { names: ["incoming_envelope"] },
  "📧": { names: ["e_mail", "email"] },
  "💌": { names: ["love_letter"] },
  "📮": { names: ["postbox"] },
  "📪": { names: ["mailbox_closed"] },
  "📫": { names: ["mailbox"] },
  "📬": { names: ["mailbox_with_mail"] },
  "📭": { names: ["mailbox_with_no_mail"] },
  "📦": { names: ["package"] },
  "📯": { names: ["postal_horn"] },
  "📥": { names: ["inbox_tray"] },
  "📤": { names: ["outbox_tray"] },
  "📜": { names: ["scroll"] },
  "📃": { names: ["page_with_curl"] },
  "📑": { names: ["bookmark_tabs"] },
  "📊": { names: ["bar_chart"] },
  "📈": { names: ["chart_with_upwards_trend"] },
  "📉": { names: ["chart_with_downwards_trend"] },
  "📄": { names: ["page_facing_up"] },
  "📅": { names: ["date"] },
  "📆": { names: ["calendar"] },
  "🗓": { names: ["calendar_spiral", "spiral_calendar_pad"] },
  "📇": { names: ["card_index"] },
  "🗃": { names: ["card_box", "card_file_box"] },
  "🗳": { names: ["ballot_box", "ballot_box_with_ballot"] },
  "🗄": { names: ["file_cabinet"] },
  "📋": { names: ["clipboard"] },
  "🗒": { names: ["notepad_spiral", "spiral_note_pad"] },
  "📁": { names: ["file_folder"] },
  "📂": { names: ["open_file_folder"] },
  "🗂": { names: ["dividers", "card_index_dividers"] },
  "🗞": { names: ["newspaper2", "rolled_up_newspaper"] },
  "📰": { names: ["newspaper"] },
  "📓": { names: ["notebook"] },
  "📕": { names: ["closed_book"] },
  "📗": { names: ["green_book"] },
  "📘": { names: ["blue_book"] },
  "📙": { names: ["orange_book"] },
  "📔": { names: ["notebook_with_decorative_cover"] },
  "📒": { names: ["ledger"] },
  "📚": { names: ["books"] },
  "📖": { names: ["book"] },
  "🔗": { names: ["link"] },
  "📎": { names: ["paperclip"] },
  "🖇": { names: ["paperclips", "linked_paperclips"] },
  "✂": { names: ["scissors"] },
  "📐": { names: ["triangular_ruler"] },
  "📏": { names: ["straight_ruler"] },
  "📌": { names: ["pushpin"] },
  "📍": { names: ["round_pushpin"] },
  "🚩": { names: ["triangular_flag_on_post"] },
  "🏳": { names: ["flag_white"] },
  "🏴": { names: ["flag_black"] },
  "🔐": { names: ["closed_lock_with_key"] },
  "🔒": { names: ["lock"] },
  "🔓": { names: ["unlock"] },
  "🔏": { names: ["lock_with_ink_pen"] },
  "🖊": { names: ["pen_ballpoint", "lower_left_ballpoint_pen"] },
  "🖋": { names: ["pen_fountain", "lower_left_fountain_pen"] },
  "✒": { names: ["black_nib"] },
  "📝": { names: ["pencil"] },
  "✏": { names: ["pencil2"] },
  "🖍": { names: ["crayon", "lower_left_crayon"] },
  "🖌": { names: ["paintbrush", "lower_left_paintbrush"] },
  "🔍": { names: ["mag"] },
  "🔎": { names: ["mag_right"] },
  "🛒": { names: ["shopping_cart", "shopping_trolley"] },
  "💯": { names: ["100"] },
  "🔢": { names: ["1234"] },
  "❤": { names: ["heart"] },
  "💛": { names: ["yellow_heart"] },
  "💚": { names: ["green_heart"] },
  "💙": { names: ["blue_heart"] },
  "💜": { names: ["purple_heart"] },
  "💔": { names: ["broken_heart"] },
  "❣": {
    names: ["heart_exclamation", "heavy_heart_exclamation_mark_ornament"],
  },
  "💕": { names: ["two_hearts"] },
  "💞": { names: ["revolving_hearts"] },
  "💓": { names: ["heartbeat"] },
  "💗": { names: ["heartpulse"] },
  "💖": { names: ["sparkling_heart"] },
  "💘": { names: ["cupid"] },
  "💝": { names: ["gift_heart"] },
  "💟": { names: ["heart_decoration"] },
  "☮": { names: ["peace", "peace_symbol"] },
  "✝": { names: ["cross", "latin_cross"] },
  "☪": { names: ["star_and_crescent"] },
  "🕉": { names: ["om_symbol"] },
  "☸": { names: ["wheel_of_dharma"] },
  "✡": { names: ["star_of_david"] },
  "🔯": { names: ["six_pointed_star"] },
  "🕎": { names: ["menorah"] },
  "☯": { names: ["yin_yang"] },
  "☦": { names: ["orthodox_cross"] },
  "🛐": { names: ["place_of_worship", "worship_symbol"] },
  "⛎": { names: ["ophiuchus"] },
  "♈": { names: ["aries"] },
  "♉": { names: ["taurus"] },
  "♊": { names: ["gemini"] },
  "♋": { names: ["cancer"] },
  "♌": { names: ["leo"] },
  "♍": { names: ["virgo"] },
  "♎": { names: ["libra"] },
  "♏": { names: ["scorpius"] },
  "♐": { names: ["sagittarius"] },
  "♑": { names: ["capricorn"] },
  "♒": { names: ["aquarius"] },
  "♓": { names: ["pisces"] },
  "🆔": { names: ["id"] },
  "⚛": { names: ["atom", "atom_symbol"] },
  "🈳": { names: ["u7a7a"] },
  "🈹": { names: ["u5272"] },
  "☢": { names: ["radioactive", "radioactive_sign"] },
  "☣": { names: ["biohazard", "biohazard_sign"] },
  "📴": { names: ["mobile_phone_off"] },
  "📳": { names: ["vibration_mode"] },
  "🈶": { names: ["u6709"] },
  "🈚": { names: ["u7121"] },
  "🈸": { names: ["u7533"] },
  "🈺": { names: ["u55b6"] },
  "🈷": { names: ["u6708"] },
  "✴": { names: ["eight_pointed_black_star"] },
  "🆚": { names: ["vs"] },
  "🉑": { names: ["accept"] },
  "💮": { names: ["white_flower"] },
  "🉐": { names: ["ideograph_advantage"] },
  "㊙": { names: ["secret"] },
  "㊗": { names: ["congratulations"] },
  "🈴": { names: ["u5408"] },
  "🈵": { names: ["u6e80"] },
  "🈲": { names: ["u7981"] },
  "🅰": { names: ["a"] },
  "🅱": { names: ["b"] },
  "🆎": { names: ["ab"] },
  "🆑": { names: ["cl"] },
  "🅾": { names: ["o2"] },
  "🆘": { names: ["sos"] },
  "⛔": { names: ["no_entry"] },
  "📛": { names: ["name_badge"] },
  "🚫": { names: ["no_entry_sign"] },
  "❌": { names: ["x"] },
  "⭕": { names: ["o"] },
  "💢": { names: ["anger"] },
  "♨": { names: ["hotsprings"] },
  "🚷": { names: ["no_pedestrians"] },
  "🚯": { names: ["do_not_litter"] },
  "🚳": { names: ["no_bicycles"] },
  "🚱": { names: ["non_potable_water"] },
  "🔞": { names: ["underage"] },
  "📵": { names: ["no_mobile_phones"] },
  "❗": { names: ["exclamation"] },
  "❕": { names: ["grey_exclamation"] },
  "❓": { names: ["question"] },
  "❔": { names: ["grey_question"] },
  "‼": { names: ["bangbang"] },
  "⁉": { names: ["interrobang"] },
  "🔅": { names: ["low_brightness"] },
  "🔆": { names: ["high_brightness"] },
  "🔱": { names: ["trident"] },
  "⚜": { names: ["fleur_de_lis"] },
  "〽": { names: ["part_alternation_mark"] },
  "⚠": { names: ["warning"] },
  "🚸": { names: ["children_crossing"] },
  "🔰": { names: ["beginner"] },
  "♻": { names: ["recycle"] },
  "🈯": { names: ["u6307"] },
  "💹": { names: ["chart"] },
  "❇": { names: ["sparkle"] },
  "✳": { names: ["eight_spoked_asterisk"] },
  "❎": { names: ["negative_squared_cross_mark"] },
  "✅": { names: ["white_check_mark"] },
  "💠": { names: ["diamond_shape_with_a_dot_inside"] },
  "🌀": { names: ["cyclone"] },
  "➿": { names: ["loop"] },
  "🌐": { names: ["globe_with_meridians"] },
  "Ⓜ": { names: ["m"] },
  "🏧": { names: ["atm"] },
  "🈂": { names: ["sa"] },
  "🛂": { names: ["passport_control"] },
  "🛃": { names: ["customs"] },
  "🛄": { names: ["baggage_claim"] },
  "🛅": { names: ["left_luggage"] },
  "♿": { names: ["wheelchair"] },
  "🚭": { names: ["no_smoking"] },
  "🚾": { names: ["wc"] },
  "🅿": { names: ["parking"] },
  "🚰": { names: ["potable_water"] },
  "🚹": { names: ["mens"] },
  "🚺": { names: ["womens"] },
  "🚼": { names: ["baby_symbol"] },
  "🚻": { names: ["restroom"] },
  "🚮": { names: ["put_litter_in_its_place"] },
  "🎦": { names: ["cinema"] },
  "📶": { names: ["signal_strength"] },
  "🈁": { names: ["koko"] },
  "🆖": { names: ["ng"] },
  "🆗": { names: ["ok"] },
  "🆙": { names: ["up"] },
  "🆒": { names: ["cool"] },
  "🆕": { names: ["new"] },
  "🆓": { names: ["free"] },
  "0⃣": { names: ["zero"] },
  "1⃣": { names: ["one"] },
  "2⃣": { names: ["two"] },
  "3⃣": { names: ["three"] },
  "4⃣": { names: ["four"] },
  "5⃣": { names: ["five"] },
  "6⃣": { names: ["six"] },
  "7⃣": { names: ["seven"] },
  "8⃣": { names: ["eight"] },
  "9⃣": { names: ["nine"] },
  "🔟": { names: ["keycap_ten"] },
  "▶": { names: ["arrow_forward"] },
  "⏸": { names: ["pause_button", "double_vertical_bar"] },
  "⏯": { names: ["play_pause"] },
  "⏹": { names: ["stop_button"] },
  "⏺": { names: ["record_button"] },
  "⏭": { names: ["track_next", "next_track"] },
  "⏮": { names: ["track_previous", "previous_track"] },
  "⏩": { names: ["fast_forward"] },
  "⏪": { names: ["rewind"] },
  "🔀": { names: ["twisted_rightwards_arrows"] },
  "🔁": { names: ["repeat"] },
  "🔂": { names: ["repeat_one"] },
  "◀": { names: ["arrow_backward"] },
  "🔼": { names: ["arrow_up_small"] },
  "🔽": { names: ["arrow_down_small"] },
  "⏫": { names: ["arrow_double_up"] },
  "⏬": { names: ["arrow_double_down"] },
  "➡": { names: ["arrow_right"] },
  "⬅": { names: ["arrow_left"] },
  "⬆": { names: ["arrow_up"] },
  "⬇": { names: ["arrow_down"] },
  "↗": { names: ["arrow_upper_right"] },
  "↘": { names: ["arrow_lower_right"] },
  "↙": { names: ["arrow_lower_left"] },
  "↖": { names: ["arrow_upper_left"] },
  "↕": { names: ["arrow_up_down"] },
  "↔": { names: ["left_right_arrow"] },
  "🔄": { names: ["arrows_counterclockwise"] },
  "↪": { names: ["arrow_right_hook"] },
  "↩": { names: ["leftwards_arrow_with_hook"] },
  "⤴": { names: ["arrow_heading_up"] },
  "⤵": { names: ["arrow_heading_down"] },
  "#⃣": { names: ["hash"] },
  "*⃣": { names: ["asterisk", "keycap_asterisk"] },
  "ℹ": { names: ["information_source"] },
  "🔤": { names: ["abc"] },
  "🔡": { names: ["abcd"] },
  "🔠": { names: ["capital_abcd"] },
  "🔣": { names: ["symbols"] },
  "🎵": { names: ["musical_note"] },
  "🎶": { names: ["notes"] },
  "〰": { names: ["wavy_dash"] },
  "➰": { names: ["curly_loop"] },
  "✔": { names: ["heavy_check_mark"] },
  "🔃": { names: ["arrows_clockwise"] },
  "➕": { names: ["heavy_plus_sign"] },
  "➖": { names: ["heavy_minus_sign"] },
  "➗": { names: ["heavy_division_sign"] },
  "✖": { names: ["heavy_multiplication_x"] },
  "💲": { names: ["heavy_dollar_sign"] },
  "💱": { names: ["currency_exchange"] },
  "©": { names: ["copyright"] },
  "®": { names: ["registered"] },
  "™": { names: ["tm"] },
  "🔚": { names: ["end"] },
  "🔙": { names: ["back"] },
  "🔛": { names: ["on"] },
  "🔝": { names: ["top"] },
  "🔜": { names: ["soon"] },
  "☑": { names: ["ballot_box_with_check"] },
  "🔘": { names: ["radio_button"] },
  "⚪": { names: ["white_circle"] },
  "⚫": { names: ["black_circle"] },
  "🔴": { names: ["red_circle"] },
  "🔵": { names: ["large_blue_circle"] },
  "🔸": { names: ["small_orange_diamond"] },
  "🔹": { names: ["small_blue_diamond"] },
  "🔶": { names: ["large_orange_diamond"] },
  "🔷": { names: ["large_blue_diamond"] },
  "🔺": { names: ["small_red_triangle"] },
  "▪": { names: ["black_small_square"] },
  "▫": { names: ["white_small_square"] },
  "⬛": { names: ["black_large_square"] },
  "⬜": { names: ["white_large_square"] },
  "🔻": { names: ["small_red_triangle_down"] },
  "◼": { names: ["black_medium_square"] },
  "◻": { names: ["white_medium_square"] },
  "◾": { names: ["black_medium_small_square"] },
  "◽": { names: ["white_medium_small_square"] },
  "🔲": { names: ["black_square_button"] },
  "🔳": { names: ["white_square_button"] },
  "🔈": { names: ["speaker"] },
  "🔉": { names: ["sound"] },
  "🔊": { names: ["loud_sound"] },
  "🔇": { names: ["mute"] },
  "📣": { names: ["mega"] },
  "📢": { names: ["loudspeaker"] },
  "🔔": { names: ["bell"] },
  "🔕": { names: ["no_bell"] },
  "🃏": { names: ["black_joker"] },
  "🀄": { names: ["mahjong"] },
  "♠": { names: ["spades"] },
  "♣": { names: ["clubs"] },
  "♥": { names: ["hearts"] },
  "♦": { names: ["diamonds"] },
  "🎴": { names: ["flower_playing_cards"] },
  "💭": { names: ["thought_balloon"] },
  "🗯": { names: ["anger_right", "right_anger_bubble"] },
  "💬": { names: ["speech_balloon"] },
  "🕐": { names: ["clock1"] },
  "🕑": { names: ["clock2"] },
  "🕒": { names: ["clock3"] },
  "🕓": { names: ["clock4"] },
  "🕔": { names: ["clock5"] },
  "🕕": { names: ["clock6"] },
  "🕖": { names: ["clock7"] },
  "🕗": { names: ["clock8"] },
  "🕘": { names: ["clock9"] },
  "🕙": { names: ["clock10"] },
  "🕚": { names: ["clock11"] },
  "🕛": { names: ["clock12"] },
  "🕜": { names: ["clock130"] },
  "🕝": { names: ["clock230"] },
  "🕞": { names: ["clock330"] },
  "🕟": { names: ["clock430"] },
  "🕠": { names: ["clock530"] },
  "🕡": { names: ["clock630"] },
  "🕢": { names: ["clock730"] },
  "🕣": { names: ["clock830"] },
  "🕤": { names: ["clock930"] },
  "🕥": { names: ["clock1030"] },
  "🕦": { names: ["clock1130"] },
  "🕧": { names: ["clock1230"] },
  "👁‍🗨": { names: ["eye_in_speech_bubble"] },
  "🗨": { names: ["speech_left", "left_speech_bubble"] },
  "⏏": { names: ["eject", "eject_symbol"] },
  "🖤": { names: ["black_heart"] },
  "🛑": { names: ["octagonal_sign", "stop_sign"] },
  "🇿": { names: ["regional_indicator_z"] },
  "🇾": { names: ["regional_indicator_y"] },
  "🇽": { names: ["regional_indicator_x"] },
  "🇼": { names: ["regional_indicator_w"] },
  "🇻": { names: ["regional_indicator_v"] },
  "🇺": { names: ["regional_indicator_u"] },
  "🇹": { names: ["regional_indicator_t"] },
  "🇸": { names: ["regional_indicator_s"] },
  "🇷": { names: ["regional_indicator_r"] },
  "🇶": { names: ["regional_indicator_q"] },
  "🇵": { names: ["regional_indicator_p"] },
  "🇴": { names: ["regional_indicator_o"] },
  "🇳": { names: ["regional_indicator_n"] },
  "🇲": { names: ["regional_indicator_m"] },
  "🇱": { names: ["regional_indicator_l"] },
  "🇰": { names: ["regional_indicator_k"] },
  "🇯": { names: ["regional_indicator_j"] },
  "🇮": { names: ["regional_indicator_i"] },
  "🇭": { names: ["regional_indicator_h"] },
  "🇬": { names: ["regional_indicator_g"] },
  "🇫": { names: ["regional_indicator_f"] },
  "🇪": { names: ["regional_indicator_e"] },
  "🇩": { names: ["regional_indicator_d"] },
  "🇨": { names: ["regional_indicator_c"] },
  "🇧": { names: ["regional_indicator_b"] },
  "🇦": { names: ["regional_indicator_a"] },
  "🇦🇨": { names: ["flag_ac"] },
  "🇦🇫": { names: ["flag_af"] },
  "🇦🇱": { names: ["flag_al"] },
  "🇩🇿": { names: ["flag_dz"] },
  "🇦🇩": { names: ["flag_ad"] },
  "🇦🇴": { names: ["flag_ao"] },
  "🇦🇮": { names: ["flag_ai"] },
  "🇦🇬": { names: ["flag_ag"] },
  "🇦🇷": { names: ["flag_ar"] },
  "🇦🇲": { names: ["flag_am"] },
  "🇦🇼": { names: ["flag_aw"] },
  "🇦🇺": { names: ["flag_au"] },
  "🇦🇹": { names: ["flag_at"] },
  "🇦🇿": { names: ["flag_az"] },
  "🇧🇸": { names: ["flag_bs"] },
  "🇧🇭": { names: ["flag_bh"] },
  "🇧🇩": { names: ["flag_bd"] },
  "🇧🇧": { names: ["flag_bb"] },
  "🇧🇾": { names: ["flag_by"] },
  "🇧🇪": { names: ["flag_be"] },
  "🇧🇿": { names: ["flag_bz"] },
  "🇧🇯": { names: ["flag_bj"] },
  "🇧🇲": { names: ["flag_bm"] },
  "🇧🇹": { names: ["flag_bt"] },
  "🇧🇴": { names: ["flag_bo"] },
  "🇧🇦": { names: ["flag_ba"] },
  "🇧🇼": { names: ["flag_bw"] },
  "🇧🇷": { names: ["flag_br"] },
  "🇧🇳": { names: ["flag_bn"] },
  "🇧🇬": { names: ["flag_bg"] },
  "🇧🇫": { names: ["flag_bf"] },
  "🇧🇮": { names: ["flag_bi"] },
  "🇨🇻": { names: ["flag_cv"] },
  "🇰🇭": { names: ["flag_kh"] },
  "🇨🇲": { names: ["flag_cm"] },
  "🇨🇦": { names: ["flag_ca"] },
  "🇰🇾": { names: ["flag_ky"] },
  "🇨🇫": { names: ["flag_cf"] },
  "🇹🇩": { names: ["flag_td"] },
  "🇨🇱": { names: ["flag_cl"] },
  "🇨🇳": { names: ["flag_cn"] },
  "🇨🇴": { names: ["flag_co"] },
  "🇰🇲": { names: ["flag_km"] },
  "🇨🇬": { names: ["flag_cg"] },
  "🇨🇩": { names: ["flag_cd"] },
  "🇨🇷": { names: ["flag_cr"] },
  "🇭🇷": { names: ["flag_hr"] },
  "🇨🇺": { names: ["flag_cu"] },
  "🇨🇾": { names: ["flag_cy"] },
  "🇨🇿": { names: ["flag_cz"] },
  "🇩🇰": { names: ["flag_dk"] },
  "🇩🇯": { names: ["flag_dj"] },
  "🇩🇲": { names: ["flag_dm"] },
  "🇩🇴": { names: ["flag_do"] },
  "🇪🇨": { names: ["flag_ec"] },
  "🇪🇬": { names: ["flag_eg"] },
  "🇸🇻": { names: ["flag_sv"] },
  "🇬🇶": { names: ["flag_gq"] },
  "🇪🇷": { names: ["flag_er"] },
  "🇪🇪": { names: ["flag_ee"] },
  "🇪🇹": { names: ["flag_et"] },
  "🇫🇰": { names: ["flag_fk"] },
  "🇫🇴": { names: ["flag_fo"] },
  "🇫🇯": { names: ["flag_fj"] },
  "🇫🇮": { names: ["flag_fi"] },
  "🇫🇷": { names: ["flag_fr"] },
  "🇵🇫": { names: ["flag_pf"] },
  "🇬🇦": { names: ["flag_ga"] },
  "🇬🇲": { names: ["flag_gm"] },
  "🇬🇪": { names: ["flag_ge"] },
  "🇩🇪": { names: ["flag_de"] },
  "🇬🇭": { names: ["flag_gh"] },
  "🇬🇮": { names: ["flag_gi"] },
  "🇬🇷": { names: ["flag_gr"] },
  "🇬🇱": { names: ["flag_gl"] },
  "🇬🇩": { names: ["flag_gd"] },
  "🇬🇺": { names: ["flag_gu"] },
  "🇬🇹": { names: ["flag_gt"] },
  "🇬🇳": { names: ["flag_gn"] },
  "🇬🇼": { names: ["flag_gw"] },
  "🇬🇾": { names: ["flag_gy"] },
  "🇭🇹": { names: ["flag_ht"] },
  "🇭🇳": { names: ["flag_hn"] },
  "🇭🇰": { names: ["flag_hk"] },
  "🇭🇺": { names: ["flag_hu"] },
  "🇮🇸": { names: ["flag_is"] },
  "🇮🇳": { names: ["flag_in"] },
  "🇮🇩": { names: ["flag_id"] },
  "🇮🇷": { names: ["flag_ir"] },
  "🇮🇶": { names: ["flag_iq"] },
  "🇮🇪": { names: ["flag_ie"] },
  "🇮🇱": { names: ["flag_il"] },
  "🇮🇹": { names: ["flag_it"] },
  "🇨🇮": { names: ["flag_ci"] },
  "🇯🇲": { names: ["flag_jm"] },
  "🇯🇵": { names: ["flag_jp"] },
  "🇯🇪": { names: ["flag_je"] },
  "🇯🇴": { names: ["flag_jo"] },
  "🇰🇿": { names: ["flag_kz"] },
  "🇰🇪": { names: ["flag_ke"] },
  "🇰🇮": { names: ["flag_ki"] },
  "🇽🇰": { names: ["flag_xk"] },
  "🇰🇼": { names: ["flag_kw"] },
  "🇰🇬": { names: ["flag_kg"] },
  "🇱🇦": { names: ["flag_la"] },
  "🇱🇻": { names: ["flag_lv"] },
  "🇱🇧": { names: ["flag_lb"] },
  "🇱🇸": { names: ["flag_ls"] },
  "🇱🇷": { names: ["flag_lr"] },
  "🇱🇾": { names: ["flag_ly"] },
  "🇱🇮": { names: ["flag_li"] },
  "🇱🇹": { names: ["flag_lt"] },
  "🇱🇺": { names: ["flag_lu"] },
  "🇲🇴": { names: ["flag_mo"] },
  "🇲🇰": { names: ["flag_mk"] },
  "🇲🇬": { names: ["flag_mg"] },
  "🇲🇼": { names: ["flag_mw"] },
  "🇲🇾": { names: ["flag_my"] },
  "🇲🇻": { names: ["flag_mv"] },
  "🇲🇱": { names: ["flag_ml"] },
  "🇲🇹": { names: ["flag_mt"] },
  "🇲🇭": { names: ["flag_mh"] },
  "🇲🇷": { names: ["flag_mr"] },
  "🇲🇺": { names: ["flag_mu"] },
  "🇲🇽": { names: ["flag_mx"] },
  "🇫🇲": { names: ["flag_fm"] },
  "🇲🇩": { names: ["flag_md"] },
  "🇲🇨": { names: ["flag_mc"] },
  "🇲🇳": { names: ["flag_mn"] },
  "🇲🇪": { names: ["flag_me"] },
  "🇲🇸": { names: ["flag_ms"] },
  "🇲🇦": { names: ["flag_ma"] },
  "🇲🇿": { names: ["flag_mz"] },
  "🇲🇲": { names: ["flag_mm"] },
  "🇳🇦": { names: ["flag_na"] },
  "🇳🇷": { names: ["flag_nr"] },
  "🇳🇵": { names: ["flag_np"] },
  "🇳🇱": { names: ["flag_nl"] },
  "🇳🇨": { names: ["flag_nc"] },
  "🇳🇿": { names: ["flag_nz"] },
  "🇳🇮": { names: ["flag_ni"] },
  "🇳🇪": { names: ["flag_ne"] },
  "🇳🇬": { names: ["flag_ng"] },
  "🇳🇺": { names: ["flag_nu"] },
  "🇰🇵": { names: ["flag_kp"] },
  "🇳🇴": { names: ["flag_no"] },
  "🇴🇲": { names: ["flag_om"] },
  "🇵🇰": { names: ["flag_pk"] },
  "🇵🇼": { names: ["flag_pw"] },
  "🇵🇸": { names: ["flag_ps"] },
  "🇵🇦": { names: ["flag_pa"] },
  "🇵🇬": { names: ["flag_pg"] },
  "🇵🇾": { names: ["flag_py"] },
  "🇵🇪": { names: ["flag_pe"] },
  "🇵🇭": { names: ["flag_ph"] },
  "🇵🇱": { names: ["flag_pl"] },
  "🇵🇹": { names: ["flag_pt"] },
  "🇵🇷": { names: ["flag_pr"] },
  "🇶🇦": { names: ["flag_qa"] },
  "🇷🇴": { names: ["flag_ro"] },
  "🇷🇺": { names: ["flag_ru"] },
  "🇷🇼": { names: ["flag_rw"] },
  "🇸🇭": { names: ["flag_sh"] },
  "🇰🇳": { names: ["flag_kn"] },
  "🇱🇨": { names: ["flag_lc"] },
  "🇻🇨": { names: ["flag_vc"] },
  "🇼🇸": { names: ["flag_ws"] },
  "🇸🇲": { names: ["flag_sm"] },
  "🇸🇹": { names: ["flag_st"] },
  "🇸🇦": { names: ["flag_sa"] },
  "🇸🇳": { names: ["flag_sn"] },
  "🇷🇸": { names: ["flag_rs"] },
  "🇸🇨": { names: ["flag_sc"] },
  "🇸🇱": { names: ["flag_sl"] },
  "🇸🇬": { names: ["flag_sg"] },
  "🇸🇰": { names: ["flag_sk"] },
  "🇸🇮": { names: ["flag_si"] },
  "🇸🇧": { names: ["flag_sb"] },
  "🇸🇴": { names: ["flag_so"] },
  "🇿🇦": { names: ["flag_za"] },
  "🇰🇷": { names: ["flag_kr"] },
  "🇪🇸": { names: ["flag_es"] },
  "🇱🇰": { names: ["flag_lk"] },
  "🇸🇩": { names: ["flag_sd"] },
  "🇸🇷": { names: ["flag_sr"] },
  "🇸🇿": { names: ["flag_sz"] },
  "🇸🇪": { names: ["flag_se"] },
  "🇨🇭": { names: ["flag_ch"] },
  "🇸🇾": { names: ["flag_sy"] },
  "🇹🇼": { names: ["flag_tw"] },
  "🇹🇯": { names: ["flag_tj"] },
  "🇹🇿": { names: ["flag_tz"] },
  "🇹🇭": { names: ["flag_th"] },
  "🇹🇱": { names: ["flag_tl"] },
  "🇹🇬": { names: ["flag_tg"] },
  "🇹🇴": { names: ["flag_to"] },
  "🇹🇹": { names: ["flag_tt"] },
  "🇹🇳": { names: ["flag_tn"] },
  "🇹🇷": { names: ["flag_tr"] },
  "🇹🇲": { names: ["flag_tm"] },
  "🇹🇻": { names: ["flag_tv"] },
  "🇺🇬": { names: ["flag_ug"] },
  "🇺🇦": { names: ["flag_ua"] },
  "🇦🇪": { names: ["flag_ae"] },
  "🇬🇧": { names: ["flag_gb"] },
  "🇺🇸": { names: ["flag_us"] },
  "🇻🇮": { names: ["flag_vi"] },
  "🇺🇾": { names: ["flag_uy"] },
  "🇺🇿": { names: ["flag_uz"] },
  "🇻🇺": { names: ["flag_vu"] },
  "🇻🇦": { names: ["flag_va"] },
  "🇻🇪": { names: ["flag_ve"] },
  "🇻🇳": { names: ["flag_vn"] },
  "🇼🇫": { names: ["flag_wf"] },
  "🇪🇭": { names: ["flag_eh"] },
  "🇾🇪": { names: ["flag_ye"] },
  "🇿🇲": { names: ["flag_zm"] },
  "🇿🇼": { names: ["flag_zw"] },
  "🇷🇪": { names: ["flag_re"] },
  "🇦🇽": { names: ["flag_ax"] },
  "🇹🇦": { names: ["flag_ta"] },
  "🇮🇴": { names: ["flag_io"] },
  "🇧🇶": { names: ["flag_bq"] },
  "🇨🇽": { names: ["flag_cx"] },
  "🇨🇨": { names: ["flag_cc"] },
  "🇬🇬": { names: ["flag_gg"] },
  "🇮🇲": { names: ["flag_im"] },
  "🇾🇹": { names: ["flag_yt"] },
  "🇳🇫": { names: ["flag_nf"] },
  "🇵🇳": { names: ["flag_pn"] },
  "🇧🇱": { names: ["flag_bl"] },
  "🇵🇲": { names: ["flag_pm"] },
  "🇬🇸": { names: ["flag_gs"] },
  "🇹🇰": { names: ["flag_tk"] },
  "🇧🇻": { names: ["flag_bv"] },
  "🇭🇲": { names: ["flag_hm"] },
  "🇸🇯": { names: ["flag_sj"] },
  "🇺🇲": { names: ["flag_um"] },
  "🇮🇨": { names: ["flag_ic"] },
  "🇪🇦": { names: ["flag_ea"] },
  "🇨🇵": { names: ["flag_cp"] },
  "🇩🇬": { names: ["flag_dg"] },
  "🇦🇸": { names: ["flag_as"] },
  "🇦🇶": { names: ["flag_aq"] },
  "🇻🇬": { names: ["flag_vg"] },
  "🇨🇰": { names: ["flag_ck"] },
  "🇨🇼": { names: ["flag_cw"] },
  "🇪🇺": { names: ["flag_eu"] },
  "🇬🇫": { names: ["flag_gf"] },
  "🇹🇫": { names: ["flag_tf"] },
  "🇬🇵": { names: ["flag_gp"] },
  "🇲🇶": { names: ["flag_mq"] },
  "🇲🇵": { names: ["flag_mp"] },
  "🇸🇽": { names: ["flag_sx"] },
  "🇸🇸": { names: ["flag_ss"] },
  "🇹🇨": { names: ["flag_tc"] },
  "🇲🇫": { names: ["flag_mf"] },
  "🏳️‍🌈": { names: ["gay_pride_flag", "rainbow_flag"] },
  "🏻": { names: ["skin-tone-1"] },
  "🏼": { names: ["skin-tone-2"] },
  "🏽": { names: ["skin-tone-3"] },
  "🏾": { names: ["skin-tone-4"] },
  "🏿": { names: ["skin-tone-5"] },
}
